!function (t) {
    ...
    function e(e) {
        var r = e || window.event, h = o.call(arguments, 1), l = 0, m = 0, c = 0, p = 0;
    ...
    var i, n, r = ["wheel", "mousewheel", "DOMMouseScroll", "MozMousePixelScroll"], h = "onwheel"in document || document.documentMode >= 9 ? ["wheel"] : ["mousewheel", "DomMouseScroll", "MozMousePixelScroll"], o = Array.prototype.slice;
    if (t.event.fixHooks)for (var l = r.length; l;)t.event.fixHooks[r[--l]] = t.event.mouseHooks;
    var m = t.event.special.mousewheel = {
        version: "3.1.9", setup: function () {
            if (this.addEventListener)for (var a = h.length; a;)this.addEventListener(h[--a], e, !1); else this.onmousewheel = e;
            t.data(this, "mousewheel-line-height", m.getLineHeight(this)), t.data(this, "mousewheel-page-height", m.getPageHeight(this))
        }, teardown: function () {
            if (this.removeEventListener)for (var t = h.length; t;)this.removeEventListener(h[--t], e, !1); else this.onmousewheel = null
...
var jvm = {
    inherits: function (t, e) {
        ...
        if (t instanceof Array)for (e = 0; e < t.length; e++)t[e] < a && (a = t[e]); else for (e in t)t[e] < a && (a = t[e]);
        return a
    }, max: function (t) {
        ...
        var e = new jvm.$.Deferred, a = jvm.$("<img/>");
...
jvm.$ = jQuery, Array.prototype.indexOf || (Array.prototype.indexOf = function (t, e) {
    ...
    var s = Object(this), i = s.length >>> 0;
    if (0 === i)return -1;
    var n = +e || 0;
    ...
    var a;
    if ("object" == typeof t)for (a in t)this.properties[a] = t[a], this.applyAttr(a, t[a]); else this.properties[t] = e, this.applyAttr(t, e)
    ...
    setMin: function (t) {
        ...
        var t, e, a = 5, s = [this.clearMinValue, this.clearMaxValue], i = s[1] - s[0], n = Math.pow(10, Math.floor(Math.log(i / a) / Math.LN10)), r = a / i * n, h = [];
    ...
    var t, e, a, s, i = this.series.scale.getTicks(), n = jvm.$("<div/>").addClass("jvectormap-legend-inner");
    ...
    setAttributes: function (t, e) {
        var a, s = t;
        if ("string" == typeof t)this.elements[t] && this.elements[t].setStyle(this.params.attribute, e); else for (a in s)this.elements[a] && this.elements[a].element.setStyle(this.params.attribute, s[a])
    }, setValues: function (t) {
        var e, a, s = -Number.MAX_VALUE, i = Number.MAX_VALUE, n = {};
        if (this.scale instanceof jvm.OrdinalScale || this.scale instanceof jvm.SimpleScale)for (a in t)n[a] = t[a] ? this.scale.getValue(t[a]) : this.elements[a].element.style.initial[this.params.attribute]; else {
            if ("undefined" == typeof this.params.min || "undefined" == typeof this.params.max)for (a in t)e = parseFloat(t[a]), e > s && (s = e), i > e && (i = e);
    ...
    degRad: 180 / Math.PI, radDeg: Math.PI / 180, radius: 6381372, sgn: function (t) {
        ...
        var s = 0, i = a * this.radDeg, n = 29.5 * this.radDeg, r = 45.5 * this.radDeg, h = t * this.radDeg, o = e * this.radDeg, l = (Math.sin(n) + Math.sin(r)) / 2, m = Math.cos(n) * Math.cos(n) + 2 * l * Math.sin(n), c = l * (o - i), p = Math.sqrt(m - 2 * l * Math.sin(h)) / l, d = Math.sqrt(m - 2 * l * Math.sin(s)) / l;
        return {x: p * Math.sin(c) * this.radius, y: -(d - p * Math.cos(c)) * this.radius}
    }, aea_inv: function (t, e, a) {
        var s = t / this.radius, i = e / this.radius, n = 0, r = a * this.radDeg, h = 29.5 * this.radDeg, o = 45.5 * this.radDeg, l = (Math.sin(h) + Math.sin(o)) / 2, m = Math.cos(h) * Math.cos(h) + 2 * l * Math.sin(h), c = Math.sqrt(m - 2 * l * Math.sin(n)) / l, p = Math.sqrt(s * s + (c - i) * (c - i)), d = Math.atan(s / (c - i));
        return {lat: Math.asin((m - p * p * l * l) / (2 * l)) * this.degRad, lng: (r + d / l) * this.degRad}
    }, lcc: function (t, e, a) {
        var s = 0, i = a * this.radDeg, n = e * this.radDeg, r = 33 * this.radDeg, h = 45 * this.radDeg, o = t * this.radDeg, l = Math.log(Math.cos(r) * (1 / Math.cos(h))) / Math.log(Math.tan(Math.PI / 4 + h / 2) * (1 / Math.tan(Math.PI / 4 + r / 2))), m = Math.cos(r) * Math.pow(Math.tan(Math.PI / 4 + r / 2), l) / l, c = m * Math.pow(1 / Math.tan(Math.PI / 4 + o / 2), l), p = m * Math.pow(1 / Math.tan(Math.PI / 4 + s / 2), l);
        return {x: c * Math.sin(l * (n - i)) * this.radius, y: -(p - c * Math.cos(l * (n - i))) * this.radius}
    }, lcc_inv: function (t, e, a) {
        var s = t / this.radius, i = e / this.radius, n = 0, r = a * this.radDeg, h = 33 * this.radDeg, o = 45 * this.radDeg, l = Math.log(Math.cos(h) * (1 / Math.cos(o))) / Math.log(Math.tan(Math.PI / 4 + o / 2) * (1 / Math.tan(Math.PI / 4 + h / 2))), m = Math.cos(h) * Math.pow(Math.tan(Math.PI / 4 + h / 2), l) / l, c = m * Math.pow(1 / Math.tan(Math.PI / 4 + n / 2), l), p = this.sgn(l) * Math.sqrt(s * s + (c - i) * (c - i)), d = Math.atan(s / (c - i));
        return {
    ...
    bindContainerEvents: function () {
        ...
        this.params.panOnDrag && (this.container.mousemove(function (i) {
            ...
            var e = jvm.$(s.container).offset(), a = t.pageX - e.left, i = t.pageY - e.top, n = Math.pow(1.003, t.deltaY);
            s.tip.hide(), s.setScale(s.scale * n, a, i), t.preventDefault()
        })
    },
    bindContainerTouchEvents: function () {
        var t, e, a, s, i, n, r, h = this, o = function (o) {
    ...
    bindElementEvents: function () {
        ...
        this.container.mousemove(function () {
            ...
            var a = jvm.$(this).attr("class").baseVal || jvm.$(this).attr("class"), s = -1 === a.indexOf("jvectormap-region") ? "marker" : "region", i = jvm.$(this).attr("region" == s ? "data-code" : "data-index"), n = "region" == s ? e.regions[i].element : e.markers[i].element, r = "region" == s ? e.mapData.paths[i].name : e.markers[i].config.name || "", h = jvm.$.Event(s + "TipShow.jvectormap"), o = jvm.$.Event(s + "Over.jvectormap");
            ...
            var a = jvm.$(this).attr("class").baseVal ? jvm.$(this).attr("class").baseVal : jvm.$(this).attr("class"), s = -1 === a.indexOf("jvectormap-region") ? "marker" : "region", i = jvm.$(this).attr("region" == s ? "data-code" : "data-index"), n = jvm.$.Event(s + "Click.jvectormap"), r = "region" == s ? e.regions[i].element : e.markers[i].element;
    ...
    createTip: function () {
        ...
        this.tip = jvm.$("<div/>").addClass("jvectormap-tip").appendTo(jvm.$("body")), this.container.mousemove(function (e) {
            var a = e.pageX - 15 - t.tipWidth, s = e.pageY - 15 - t.tipHeight;
    ...
    setScale: function (t, e, a, s, i) {
        var n, r, h, o, l, m, c, p, d, u = jvm.$.Event("zoom.jvectormap"), v = this, g = 0, f = Math.abs(Math.round(60 * (t - this.scale) / Math.max(t, this.scale))), j = new jvm.$.Deferred;
    ...
    setSelected: function (t, e) {
        var a;
        if ("object" != typeof e && (e = [e]), jvm.$.isArray(e))for (a = 0; a < e.length; a++)this[t][e[a]].element.setSelected(!0); else for (a in e)this[t][a].element.setSelected(!!e[a])
    },
    ...
    createMarkers: function (t) {
        ...
        if (this.markersGroup = this.markersGroup || this.canvas.addGroup(), this.markerLabelsGroup = this.markerLabelsGroup || this.canvas.addGroup(), jvm.$.isArray(t))for (n = t.slice(), t = {}, e = 0; e < n.length; e++)t[e] = n[e];
        for (e in t)i = t[e]instanceof Array ? {latLng: t[e]} : t[e], s = this.getMarkerPosition(i), s !== !1 && (a = new jvm.Marker({
            map: this,
        ...
        }), this.markers[e] && this.removeMarkers([e]), this.markers[e] = {element: a, config: i})
    },
    ...
    latLngToPoint: function (t, e) {
        var a, s, i, n = jvm.Map.maps[this.params.map].projection, r = n.centralMeridian;
    ...
    pointToLatLng: function (t, e) {
        var a, s, i, n, r, h = jvm.Map.maps[this.params.map].projection, o = h.centralMeridian, l = jvm.Map.maps[this.params.map].insets;
        for (a = 0; a < l.length; a++)if (s = l[a], i = s.bbox, n = t - (this.transX * this.scale + s.left * this.scale), r = e - (this.transY * this.scale + s.top * this.scale), n = n / (s.width * this.scale) * (i[1].x - i[0].x) + i[0].x, r = r / (s.height * this.scale) * (i[1].y - i[0].y) + i[0].y, n > i[0].x && n < i[1].x && r > i[0].y && r < i[1].y)return jvm.Proj[h.type + "_inv"](n, -r, o);
        return !1
    },
    getInsetForPoint: function (t, e) {
        var a, s, i = jvm.Map.maps[this.params.map].insets;
        for (a = 0; a < i.length; a++)if (s = i[a].bbox, t > s[0].x && t < s[1].x && e > s[0].y && e < s[1].y)return i[a]
    },
    ...
    addMap: function (t, e) {
        ...
        var a = this.history[this.history.length - 1], s = this, i = a.setFocus({
            region: e,
            animate: !0
        ...
        var t = this.history.pop(), e = this.history[this.history.length - 1], a = this;
